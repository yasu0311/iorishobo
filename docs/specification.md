...
| 注文確定後 | カート明細を削除。**Stripe は入金完了（`paid`）時**、代引き・銀行振込は **注文作成時** |
| 会員カート掃除 | **行わない**（期限なし） |
...
|-----------|-------------------|---------------------|
| subtotal | 商品の合計金額(税込) | — |
| tax_amount | 消費税(商品合計に対する) | `floor(subtotal × rate / (1 + rate))`（`consumption_taxes`） |
| payment_fee | 決済手数料 | 0 |
| discount | 割引金額 | 0 |
...
カラーミー管理画面（決済 → 消費税設定）に相当。税率は **`consumption_taxes` テーブル**で期間管理する（コードや config に固定しない）。
...
**DB**: `consumption_taxes`（`start_date` / `end_date` / `tax_rate`）。サンプルアプリの区分 ID・ショップ別選択は不要（単一ショップのため）。`shop_settings` は作らない。店舗住所・インボイス登録番号・振込先口座は **§3.18**。

**税率変更**: 現行行の `end_date` を締め、新 `tax_rate` の行を INSERT。アプリの再デプロイは不要。
...
| Webhook 冪等 | 再送時は既に `paid` なら何もしない。`cancelled` 等の非 `pending` には入金処理しない |
| Stripe カート | 入金完了（`paid`）までカート明細を残す。代引き・振込は注文作成時に空にする |
| Stripe 二重注文防止 | 再チェックアウト送信時はセッション上の直前の `stripe + pending` をキャンセルしてから新規作成。「もう一度支払う」は既存注文で Checkout Session を再開 |
| Stripe 未完了注文 | ① 送信後にカード決済せず離脱した `pending` は一覧非表示。**作成から 7 日経過で自動キャンセル**（`payment_status` / `shipping_status` → `cancelled`）。在庫は減算前のため戻し不要。手動キャンセルも可 |
...
| 発送 | **手動**（管理画面で `partially_shipped` / `shipped` に更新。カラーミーと同様） |
...
**発送済み（`shipping_status = shipped`）および一部発送（`partially_shipped`）の注文はキャンセル不可**。返金のみ（`refunds`）。
...
| 0.47 | 2026-07-24 | Stripe は paid までカート保持、再チェックアウト時は旧 pending キャンセル、未完了 7 日自動キャンセル、cancelled への Webhook ガード |
| 0.48 | 2026-08-04 | 消費税を consumption_taxes で期間管理（コード固定を廃止） |
