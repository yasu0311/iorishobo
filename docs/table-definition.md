...
ショップ全体で共通。**商品ごとの税率列は持たない**。税率は `consumption_taxes` マスタで期間管理する。
...
1. 現行行の `end_date` を変更前日に更新
2. 新税率の行を `start_date` = 変更日、`end_date` = 十分先、`tax_rate` = 0.12 で INSERT
3. デプロイ・コード変更は不要
...
## 19. consumption_taxes（消費税率）

ショップ全体で 1 系統の標準税率のみを期間管理する（サンプルアプリのような区分 ID / ショップ別税率は持たない）。

| No | 物理名 | 論理名 | 型 | NULL | デフォルト | 説明 |
|----|--------|--------|-----|------|-----------|------|
| 1 | id | ID | BIGINT UNSIGNED | NO | AI | PK |
| 2 | start_date | 適用開始日 | DATE | NO | | この日を含む |
| 3 | end_date | 適用終了日 | DATE | NO | | この日を含む。現行行は十分先の日付 |
| 4 | tax_rate | 税率 | DECIMAL(6,4) | NO | | 例: 0.1000 = 10% |
| 5 | created_at | 作成日時 | TIMESTAMP | YES | | |
| 6 | updated_at | 更新日時 | TIMESTAMP | YES | | |

**初期データ**: 2014-04-01〜2019-09-30 = 0.08、2019-10-01〜2200-01-01 = 0.10。

**参照**: 新規注文・管理画面の金額再計算は注文日（未指定時は今日）で有効な行を使う。過去注文の領収書表示も `ordered_at` で同じ行を引く。
...
| 0.48 | 2026-08-04 | consumption_taxes を追加。税率は DB 期間管理（コード固定を廃止） |
