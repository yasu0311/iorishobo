...
| 2 | products | 商品（親） | マスタ |
| 3 | product_variants | 商品バリアント | マスタ |
| 4 | customers | 顧客 | マスタ |
| 5 | shipping_methods | 配送方法 | マスタ |
| 6 | orders | 注文 | トランザクション |
| 7 | order_items | 注文明細 | トランザクション |
| 8 | users | ユーザー | マスタ（Laravel 標準） |
...
| 9 | image_path | メイン画像 | VARCHAR(500) | YES | | |
| 10 | base_price | 基本価格 | INT UNSIGNED | NO | | 税込 |
| 11 | weight | 重量 | INT UNSIGNED | YES | | グラム |
| 12 | stock_managed | 在庫管理 | BOOLEAN | NO | | |
| 13 | is_published | 掲載 | BOOLEAN | NO | | |
| 14 | sort_order | 表示順 | INT UNSIGNED | NO | | |
| 15 | created_at | 作成日時 | TIMESTAMP | YES | | |
| 16 | updated_at | 更新日時 | TIMESTAMP | YES | | |

---

## 6. product_variants（商品バリアント）
...
| 5 | attributes | オプション属性 | JSON | YES | | 学年・教科書準拠等 |
...
| 4 | base_fee | 基本送料 | INT UNSIGNED | NO | | 円 |
| 5 | is_active | 有効 | BOOLEAN | NO | | |
| 6 | sort_order | 表示順 | INT UNSIGNED | NO | | |
...
| 11 | discount | 割引 | INT UNSIGNED | NO | | |
| 12 | total | 総合計 | INT UNSIGNED | NO | | 税込 |
| 13 | payment_method | 決済方法 | VARCHAR(50) | YES | | |
| 14 | payment_status | 入金状態 | VARCHAR(30) | YES | | |
| 15 | shipping_status | 発送状態 | VARCHAR(30) | YES | | |
| 16 | shipping_method_id | 配送方法 ID | BIGINT UNSIGNED | YES | FK | |
| 17 | customer_note | 注文備考 | TEXT | YES | | sales 列 33 |
| 18 | shipping_note | 配送先備考 | TEXT | YES | | sales 列 54 |
| 19 | stripe_payment_intent_id | Stripe PI ID | VARCHAR(255) | YES | | |
| 20 | created_at | 作成日時 | TIMESTAMP | YES | | |
| 21 | updated_at | 更新日時 | TIMESTAMP | YES | | |

### 9.2 購入者（buyer_*）— sales_all「購入者」列

| No | 物理名 | 論理名 | 型 | NULL | CSV 列 |
|----|--------|--------|-----|------|--------|
| 22 | buyer_name | 購入者氏名 | VARCHAR(100) | NO | 購入者 名前 |
| 23 | buyer_email | 購入者メール | VARCHAR(255) | NO | 購入者 メールアドレス |
| 24 | buyer_phone | 購入者電話 | VARCHAR(20) | YES | 購入者 電話番号 |
| 25 | buyer_mobile | 購入者携帯 | VARCHAR(20) | YES | 購入者 携帯番号 |
| 26 | buyer_postal_code | 購入者郵便番号 | CHAR(7) | NO | 購入者 郵便番号 |
| 27 | buyer_prefecture | 購入者都道府県 | VARCHAR(20) | NO | 購入者 都道府県 |
| 28 | buyer_address_line1 | 購入者住所 | VARCHAR(255) | NO | 購入者 住所 |
| 29 | buyer_address_line2 | 購入者建物名 | VARCHAR(255) | YES | （CSV では結合） |

※ 購入者フリガナ列は **sales_all.csv に存在しない** ため持たない。

### 9.3 配送先（shipping_*）— sales_all「配送先」列

| No | 物理名 | 論理名 | 型 | NULL | CSV 列 |
|----|--------|--------|-----|------|--------|
| 30 | shipping_name | 配送先氏名 | VARCHAR(100) | NO | 配送先 名前 |
| 31 | shipping_name_kana | 配送先フリガナ | VARCHAR(100) | YES | 配送先 フリガナ・**任意** |
| 32 | shipping_phone | 配送先電話 | VARCHAR(20) | NO | 配送先 電話番号 |
| 33 | shipping_postal_code | 配送先郵便番号 | CHAR(7) | NO | 配送先 郵便番号 |
| 34 | shipping_prefecture | 配送先都道府県 | VARCHAR(20) | NO | 配送先 都道府県名 |
| 35 | shipping_address_line1 | 配送先住所 | VARCHAR(255) | NO | 配送先 住所 |
| 36 | shipping_address_line2 | 配送先建物名 | VARCHAR(255) | YES | （CSV では結合） |
