...
会員プロフィール。**移行は `customer.csv` の「ユーザー登録=有」の行のみ**（[仕様書 §3.20](./specification.md#320-顧客会員の移行)）。ゲスト相当（無）は移行しない。FAX は持たない（実データ常に空）。
...
| 8 | max_uses | 利用上限回数 | INT UNSIGNED | YES | | 全ユーザー合計。NULL=無制限 |
| 9 | used_count | 利用回数 | INT UNSIGNED | NO | | 注文確定時に加算。デフォルト 0 |
...
| 3 | customer_id | 顧客 ID | BIGINT UNSIGNED | YES | FK | 移行: §3.20。新規: ログイン時にセット |
| 4 | user_id | ユーザー ID | BIGINT UNSIGNED | YES | FK | 移行: **常に NULL**。新規: ログイン購入時のみ。マイページ注文履歴はこの列で絞る（§3.20） |
...
| 24 | shipping_method_id | 配送方法 ID | BIGINT UNSIGNED | YES | FK | 新規注文用 |
...
| 28 | stripe_payment_intent_id | Stripe PI ID | VARCHAR(255) | YES | | `payment_method = stripe` のときのみ |
...
3. 発送後、管理画面で **発送済** に更新（`shipped_at`、必要なら `tracking_number`）。
...
移行注文は [仕様書 §3.12](./specification.md#312-決済入金発送ステータス) に従いマッピングする。`customer_id` / `user_id` は [§3.20](./specification.md#320-顧客会員の移行)。キャンセル・返金列は移行時は未使用なら 0 / NULL。

### 13.6 キャンセル・返金

| 操作 | 更新する列 | 備考 |
|------|-----------|------|
| 注文キャンセル | `cancelled_at`, `cancel_reason`, `shipping_status` → `cancelled` | 未発送なら在庫を戻す。Stripe 入金済みは返金確認（§3.17） |
...
|----|--------|--------|-----|------|------|------|
| 1 | id | ID | BIGINT UNSIGNED | NO | PK | |
| 2 | customer_id | 顧客 ID | BIGINT UNSIGNED | YES | FK | 会員のとき。ゲストのみの登録は NULL |
