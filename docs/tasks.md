...
- [○] 領収書表示（税込合計・うち消費税（注文日の税率）・インボイス登録番号）
...
> **方針:** HTML 化は見送り。トランザクションメールは現行の **テキストのみ** で運用する（注文確認・振込案内・発送通知はフェーズ 2 で実装済み）。

- [×] 注文確認メール（HTML + テキスト）— 見送り
- [×] 振込案内メール（HTML 化）— 見送り
- [×] 発送通知メール（HTML 化）— 見送り
- [×] 各クライアントでの表示確認（HTML）— 見送り
- [○] テキスト版メール（注文確認・振込案内・発送通知・お問い合わせ）— フェーズ 2 / 2.7 で実装済み
...
- [ ] 実機・主要ブラウザでの表示確認
  - [ ] ストアフロント — PC / スマホ両方
  - [ ] 管理画面 — PC のみ
- [~] アクセシビリティの最低限チェック（フォーカス・コントラスト・ラベル）※スキップリンク・`:focus-visible`・ラベルは実装済み。実機確認は未
- [○] `robots.txt`・`sitemap.xml`
- [○] OGP・ファビコン
- [○] フッター情報（店舗名・連絡先・特商法リンク等）の掲載 ※リンク・連絡先は実装済み。デザイン仕上げは 3.4
- [~] パフォーマンス確認（画像最適化・LCP 等）※商品詳細の `fetchpriority` / `lazy` 実装済み。実測は未
- [~] 静的ページ・お問い合わせの文言最終確認（事業者情報・返品規定等を `.env` / 本文に反映）※プライバシーポリシー連絡先追記済み。`.env` 反映は運用時
...
| 2026-07-02 | フェーズ 3.6 — メール HTML 化は見送り（テキスト版で運用） |
| 2026-07-02 | フェーズ 3.7 — OGP・ファビコン・スキップリンク・商品画像最適化・プライバシーポリシー連絡先 |
