...
$backupDisks = array_values(array_filter(array_map(
    trim(...),
    explode(',', (string) env('BACKUP_DISKS', env('BACKUP_DISK', 'backups')))
)));
...
// Google Drive では日本語フォルダ名で list に失敗することがあるため APP_NAME は使わない
$backupName = env('BACKUP_NAME', 'market');
...
return [
    ...
    'notifications' => [
        'notifications' => [
            \Spatie\Backup\Notifications\Notifications\BackupHasFailedNotification::class => env('BACKUP_MAIL_TO') ? ['mail'] : [],
            \Spatie\Backup\Notifications\Notifications\UnhealthyBackupWasFoundNotification::class => env('BACKUP_MAIL_TO') ? ['mail'] : [],
            \Spatie\Backup\Notifications\Notifications\CleanupHasFailedNotification::class => env('BACKUP_MAIL_TO') ? ['mail'] : [],
