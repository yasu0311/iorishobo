    ...
    <div class="invoice">
      ...
      <div class="invoice-meta">
        <div class="invoice-meta-item">
          <span class="invoice-meta-label">ステータス:</span>
      ...
      <div class="invoice-payment" style="margin-bottom: 1em;">
        <div class="invoice-payment-title">ご案内</div>
        <div class="invoice-payment-detail">決済を確認しています。確定後にダウンロード可能になります。</div>
      </div>
      @endif

      @if($order->status === 'completed' && $order->product && $order->product->productFiles->isNotEmpty())
      <section class="invoice-items" style="margin-bottom: 1.5em;">
        <div class="invoice-party-label" style="margin-bottom: 0.5em;">教材ダウンロード</div>
        @if($order->product->isAvailable())
          <p class="invoice-payment-detail" style="margin-bottom: 0.8em;">購入済みの教材ファイルをダウンロードできます。</p>
          <ul style="list-style: none; padding: 0; margin: 0;">
            @foreach($order->product->productFiles as $file)
              @if($file->sample)
                <li style="padding: 0.3em 0;"><strong>{{ $file->file_name }}</strong> <span style="font-size: 0.9em; color: #666;">（見本）</span> — <a href="{{ route('member.buy.products.download', $file) }}">ダウンロード</a></li>
              @else
                <li style="padding: 0.3em 0;"><strong>{{ $file->file_name }}</strong> — <a href="{{ route('member.buy.products.download', $file) }}">ダウンロード</a></li>
              @endif
            @endforeach
          </ul>
        @else
          <p class="invoice-payment-detail" style="margin-bottom: 0.8em; color: #666;">この商品は販売終了のため、ダウンロードはできません。</p>
        @endif
      </section>
      @endif
